    ...
    <div className="relative md:px-12">
      <div className="relative w-full flex gap-4 justify-center p-4">
        {testimonials.slice(currentIndex, currentIndex + testimonialsPerSlide).map((testimonial: any, index: any) => (
          <div
            key={index}
            className={`flex-1 border border-gray-200 rounded-2xl md:flex-none w-full md:w-1/3 p-4 transition-opacity duration-500 ${fade ? 'opacity-100' : 'opacity-0'}`}
          >
            <div className="flex flex-col text-center h-full justify-center gap-4">
              ...
              <p className="flex items-center justify-center gap-4 text-sm mt-2 text-gray-600 font-semibold">
